...
namespace BecaUserInterface
{
    public partial class SymbolLibrary : Form
    {
        ...
        public bool abortScreenOpen = false;
        ...
        public void InsertDwg(string fname)
        {
            ...
            try
            {
                if (SVON)
                {
                    object result = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.GetLispSymbol("SF");

                    if (result != null)
                    {
                        scale = Convert.ToDouble(result);
                    }
                }
            }
            catch { }
            ...
            while (loop)
            {
                ...
                using (doc.LockDocument())
                {
                    using (Transaction trx = db.TransactionManager.StartTransaction())
                    {
                        ...
                        bref.TransformBy(Matrix3d.Scaling(scale, insertPt));//22-Oct-2018
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var lst = basicCivilCustom.CivilCustomization.Where(x => x.Description.ToUpper().Contains(txtSearch.Text.ToUpper())).Select(x => x.DrawingName.Replace(".dwg", "")).ToList();

                var lstDesc = basicCivilCustom.CivilCustomization.Where(x => x.Description.ToUpper().Contains(txtSearch.Text.ToUpper())).Select(x => x.Description).ToList();
        ...
        private void chkScale_CheckedChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SVON = false;//  doc.SendStringToExecute("(SV-OFF) ", false, false, false);
                }
            }
            catch { }
        }
